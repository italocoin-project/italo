...
# Script used with Drone CI to check that a statically build italo only links against the expected
# base system libraries.  Expects to be run with pwd of the build directory.

set -o errexit

anybad=
for bin in italod italo-{wallet-{cli,rpc},gen-trusted-multisig,blockchain-{ancestry,depth,export,import,mark-spent-outputs,stats,usage}}; do
    bad=
