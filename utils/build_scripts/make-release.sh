...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/italoproject/italo/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin italo-$build-x64-$version
    zip -r italo-$build-x64-$version.zip italo-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum italo-*-x64-$version.zip
