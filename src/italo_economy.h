...
constexpr uint64_t COIN                       = (uint64_t)1000000000; // 1 ITALO = pow(10, 9)
...
constexpr uint64_t BLOCK_REWARD_HF15      = 5 * COIN;
...
// LF to be used exclusively for Italo Chainflip liquidity seeding and incentives.  See
// https://github.com/italo-project/italo-improvement-proposals/issues/24 for more details.  This ends
// after 6 months.
...
constexpr uint64_t BLOCK_REWARD_HF17      = 3'433'333'333;
constexpr uint64_t FOUNDATION_REWARD_HF18 =   133'333'333;
...
static_assert(                           SN_REWARD_HF15 + FOUNDATION_REWARD_HF18 == BLOCK_REWARD_HF17);
...
enum struct mapping_type : uint16_t
{
  session = 0,
  wallet = 1,
  italonet = 2, // the type value stored in the database; counts as 1-year when used in a buy tx.
  italonet_2years,
  italonet_5years,
  italonet_10years,
  _count,
...
constexpr bool is_italonet_type(mapping_type t) { return t >= mapping_type::italonet && t <= mapping_type::italonet_10years; }

// How many days we add per "year" of LNS italonet registration.  We slightly extend this to the 368
...
constexpr uint64_t burn_needed(uint8_t hf_version, mapping_type type)
{
  ...
  // The base amount for session/wallet/italonet-1year:
  ...
  switch (type)
  {
    ...
    case mapping_type::italonet: /* FALLTHRU */
    ...
    case mapping_type::italonet_2years: result = 2 * basic_fee; break;
    case mapping_type::italonet_5years: result = 4 * basic_fee; break;
    case mapping_type::italonet_10years: result = 6 * basic_fee; break;
  }
  return result;
}
}; // namespace lns
