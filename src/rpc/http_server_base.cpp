...
namespace cryptonote::rpc {
  ...
  std::optional<std::string> check_authorization(std::string_view auth_header, std::string_view realm, Callback check_login) {
    ...
    if (parts.size() < 2 || parts[0] != "Basic"sv || !italomq::is_base64(parts[1]))
      return fail;
    auto login = italomq::from_base64(parts[1]);
  ...
  {
    if (auto www_auth = check_authorization(req.getHeader("authorization"), "italod rpc",
  ...
  std::string http_server_base::get_remote_address(HttpResponse& res) {
    ...
    else if (addr.size() == 16)
    {
      ...
      result << "{unknown:" << italomq::to_hex(addr) << "}";
    return result.str();
  }
