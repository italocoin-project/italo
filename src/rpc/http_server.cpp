...
namespace cryptonote::rpc {
  ...
  // Invokes the actual RPC request; this is called (via italomq) from some random LMQ worker thread,
  ...
  static std::unordered_set<italomq::ItaloMQ*> timer_started;
  ...
  void http_server::start()
  {
    ...
    m_server_header = "italod/"s + (m_restricted ? std::to_string(ITALO_VERSION[0]) : std::string{ITALO_VERSION_FULL})
