...
using italomq::AuthLevel;

namespace {

const command_line::arg_descriptor<std::vector<std::string>> arg_lmq_public{
  "lmq-public",
  "Adds a public, unencrypted ItaloMQ RPC listener (with restricted capabilities) at the given "
...
const command_line::arg_descriptor<std::vector<std::string>> arg_lmq_curve_public{
  "lmq-curve-public",
  "Adds a curve-encrypted ItaloMQ RPC listener at the given address that accepts (restricted) rpc "
...
const command_line::arg_descriptor<std::vector<std::string>> arg_lmq_curve{
  "lmq-curve",
  "Adds a curve-encrypted ItaloMQ RPC listener at the given address that only accepts client connections from whitelisted client x25519 pubkeys. "
...
const command_line::arg_descriptor<std::vector<std::string>> arg_lmq_local_control{
  "lmq-local-control",
  "Adds an unencrypted ItaloMQ RPC listener with full, unrestricted capabilities and no authentication at the given address. "
#ifndef _WIN32
    "Listens at ipc://<data-dir>/italod.sock if not specified. Specify 'none' to disable the default. "
#endif
...
auto as_x_pubkeys(const std::vector<std::string>& pk_strings) {
  ...
  for (const auto& pkstr : pk_strings) {
    if (pkstr.size() != 64 || !italomq::is_hex(pkstr))
      throw std::runtime_error("Invalid LMQ login pubkey: '" + pkstr + "'; expected 64-char hex pubkey");
    pks.emplace_back();
    italomq::to_hex(pkstr.begin(), pkstr.end(), reinterpret_cast<char *>(&pks.back()));
  }
  return pks;
}
  ...
  : core_{core}, rpc_{rpc}
{
...
#ifndef _WIN32
    locals.push_back("ipc://" + core.get_config_directory().u8string() + "/italod.sock");
#endif
  ...
  for (auto& cmd : rpc_commands) {
    lmq.add_request_command(cmd.second->is_public ? "rpc" : "admin", cmd.first,
        [name=std::string_view{cmd.first}, &call=*cmd.second, this](italomq::Message& m) {
      if (m.data.size() > 1)
  ...
  //
  lmq.add_request_command("sub", "mempool", [this](italomq::Message& m) {
  ...
  // encoded block hash).
  lmq.add_request_command("sub", "block", [this](italomq::Message& m) {
      std::unique_lock lock{subs_mutex_};
    auto expiry = std::chrono::steady_clock::now() + 30min;
...
static void send_notifies(Mutex& mutex, Subs& subs, const char* desc, Call call) {
  std::vector<italomq::ConnectionID> remove;
  {
