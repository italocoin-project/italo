#ifndef ITALO_NAME_SYSTEM_H
#define ITALO_NAME_SYSTEM_H
...
struct tx_extra_italo_name_system;
class Blockchain;
}; // namespace cryptonote
...
constexpr size_t ITALONET_DOMAIN_NAME_MAX          = 63 + 5; // DNS components name must be at most 63 (+ 5 for .italo); this limit applies if there is at least one hyphen (and thus includes punycode)
constexpr size_t ITALONET_DOMAIN_NAME_MAX_NOHYPHEN = 32 + 5; // If the name does not contain a - then we restrict it to 32 characters so that it cannot be (and is obviously not) an encoded .italo address (52 characters)
constexpr size_t ITALONET_ADDRESS_BINARY_LENGTH    = sizeof(crypto::ed25519_public_key);
...
{
  static size_t constexpr BUFFER_SIZE = std::max({WALLET_ACCOUNT_BINARY_LENGTH, ITALONET_ADDRESS_BINARY_LENGTH, SESSION_PUBLIC_KEY_BINARY_LENGTH}) + SODIUM_ENCRYPTION_EXTRA_BYTES;
  ...
  //                   italonet, 52 character base32z string of an ed25519 public key
...
inline std::ostream &operator<<(std::ostream &os, mapping_value const &v) { return os << italomq::to_hex(v.to_view()); }

inline std::string_view mapping_type_str(mapping_type type)
{
  switch(type)
  {
    case mapping_type::italonet:         return "italonet"sv; // general type stored in the database; 1 year when in a purchase tx
    case mapping_type::italonet_2years:  return "italonet_2years"sv;  // Only used in a buy tx, not in the DB
    case mapping_type::italonet_5years:  return "italonet_5years"sv;  // "
    case mapping_type::italonet_10years: return "italonet_10years"sv; // "
...
constexpr bool mapping_type_allowed(uint8_t hf_version, mapping_type type) {
  return (type == mapping_type::session && hf_version >= cryptonote::network_version_15_lns)
      || (is_italonet_type(type) && hf_version >= cryptonote::network_version_16_pulse);
}

// Returns all mapping types supported for lookup as of the given hardfork.  (Note that this does
// not return the dedicated length types such as mapping_type::italonet_5years as those are only
...
/// type.  In particularly this maps all mapping_type::italonet_Xyears values to the underlying value
/// of mapping_type::italonet.
constexpr uint16_t db_mapping_type(lns::mapping_type type) {
  if (is_italonet_type(type))
    return static_cast<uint16_t>(mapping_type::italonet);
...
// Currently accepts "session" or "italonet" for lookups, buys, updates, and renewals; for buys and renewals also accepts "italonet_Ny[ear]" for N=2,5,10
...
struct name_system_db
{
  ...
  bool                        save_mapping   (crypto::hash const &tx_hash, cryptonote::tx_extra_italo_name_system const &src, uint64_t height, std::optional<uint64_t> expiration_height, int64_t owner_id, std::optional<int64_t> backup_owner_id);
  ...
  bool validate_lns_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_italo_name_system &entry, std::string *reason);
