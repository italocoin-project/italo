...
#define ITALO_DEFAULT_LOG_CATEGORY "pulse"
...
void pulse::handle_message(void *quorumnet_state, pulse::message const &msg)
{
  ...
  // TODO(italo): We don't support messages from future rounds. A round
...
Yes +-----[Block can not be added to blockchain]
  ...
  Wait For Next Block:
    ...
    - // TODO(italo): After the Genesis Pulse Block is checkpointed, we can
  ...
  if (timed_out || all_values)
  {
    ...
    crypto::hash final_hash = {};
    {
      ...
      for (size_t index = 0; index < quorum.size(); index++)
      {
        ...
        {
          epee::wipeable_string string = italomq::to_hex(tools::view_guts(random_value->data));
    ...
    MINFO(log_prefix(context) << "Block final random value " << italomq::to_hex(tools::view_guts(final_block.pulse.random_value.data)) << " generated from validators " << bitset_view16(stage.bitset));
