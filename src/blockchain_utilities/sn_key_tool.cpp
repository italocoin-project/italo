...
generate [--overwrite] FILENAME
    ...
    For an active service node this file is named `key_ed25519` in the italod
    data directory.
...
legacy [--overwrite] FILENAME
    ...
    Note that legacy keypairs are not needed as of Italo 8.x; you can use just a
...
int generate(bool ed25519, std::list<std::string_view> args) {
    ...
    if (pubkey_pos != std::string::npos)
        filename.replace(pubkey_pos, 6, italomq::to_hex(pubkey.begin(), pubkey.end()));
    ...
    if (ed25519) {
        ...
        std::cout <<
              "Public key:      " << italomq::to_hex(pubkey.begin(), pubkey.end()) <<
            "\nX25519 pubkey:   " << italomq::to_hex(x_pubkey.begin(), x_pubkey.end()) <<
            "\nItalonet address: " << italomq::to_base32z(pubkey.begin(), pubkey.end()) << ".snode\n";
    } else {
        std::cout << "Public key: " << italomq::to_hex(pubkey.begin(), pubkey.end()) << "\n";
    }
...
int show(std::list<std::string_view> args) {
    ...
    if (legacy) {
        ...
        std::cout << filename.u8string() << " (legacy SN keypair)" << "\n==========" <<
            "\nPrivate key: " << italomq::to_hex(seckey.begin(), seckey.begin() + 32) <<
            "\nPublic key:  " << italomq::to_hex(pubkey.begin(), pubkey.end()) << "\n\n";
        return 0;
    }
    ...
    if (size >= 64 && ustring_view{pubkey.data(), pubkey.size()} != ustring_view{seckey.data() + 32, 32})
        return error(13, "Error: derived pubkey (" + italomq::to_hex(pubkey.begin(), pubkey.end()) + ")"
                " != embedded pubkey (" + italomq::to_hex(seckey.begin() + 32, seckey.end()) + ")");
    if (0 != crypto_sign_ed25519_pk_to_curve25519(x_pubkey.data(), pubkey.data()))
    ...
    std::cout << filename << " (Ed25519 SN keypair)" << "\n==========" <<
        "\nSecret key:      " << italomq::to_hex(seckey.begin(), seckey.begin() + 32) <<
        "\nPublic key:      " << italomq::to_hex(pubkey.begin(), pubkey.end()) <<
        "\nX25519 pubkey:   " << italomq::to_hex(x_pubkey.begin(), x_pubkey.end()) <<
        "\nItalonet address: " << italomq::to_base32z(pubkey.begin(), pubkey.end()) << ".snode\n\n";
    return 0;
...
int restore(bool ed25519, std::list<std::string_view> args) {
    ...
    if (!(skey_hex.size() == 64 || skey_hex.size() == 128) || !italomq::is_hex(skey_hex))
    ...
    italomq::from_hex(skey_hex.begin(), skey_hex.begin() + 64, skey.begin());
    if (skey_hex.size() == 128)
        italomq::from_hex(skey_hex.begin() + 64, skey_hex.end(), pubkey_expected.emplace().begin());
    ...
    if (ed25519) {
        ...
        if (0 != crypto_sign_ed25519_pk_to_curve25519(x_pubkey.data(), pubkey.data()))
            return error(14, "Unable to convert Ed25519 pubkey to X25519 pubkey; is this a really valid secret key?");
        std::cout << "X25519 pubkey:   " << italomq::to_hex(x_pubkey.begin(), x_pubkey.end()) <<
            "\nItalonet address: " << italomq::to_base32z(pubkey.begin(), pubkey.end()) << ".snode";
    }

    if (pubkey_expected) {
        if (*pubkey_expected != pubkey)
            return error(2, "Derived pubkey (" + italomq::to_hex(pubkey.begin(), pubkey.end()) + ") doesn't match "
                    "provided pubkey (" + italomq::to_hex(pubkey_expected->begin(), pubkey_expected->end()) + ")");
    } else {
