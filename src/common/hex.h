...
namespace tools {
  ...
  bool hex_to_type(std::string_view hex, T& x) {
    if (!italomq::is_hex(hex) || hex.size() != 2*sizeof(T))
      return false;
    italomq::from_hex(hex.begin(), hex.end(), reinterpret_cast<char*>(&x));
    return true;
  }
  ...
  std::string type_to_hex(const T& val) {
    return italomq::to_hex(std::string_view{reinterpret_cast<const char*>(&val), sizeof(val)});
  }
}
