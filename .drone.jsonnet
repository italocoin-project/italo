...
local gui_wallet_step(image, wine=false) = {
    ...
    commands: [
        ...
        'git clone https://github.com/italo-project/italo-electron-gui-wallet.git',
        'cp -v build/bin/italod' + (if wine then '.exe' else '') + ' italo-electron-gui-wallet/bin',
        'cp -v build/bin/italo-wallet-rpc' + (if wine then '.exe' else '') + ' italo-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
    ...
    commands: [
        'git clone https://github.com/italo-project/italo-electron-gui-wallet.git',
        'cp -v build/bin/{italod,italo-wallet-rpc} italo-electron-gui-wallet/bin',
        'cd italo-electron-gui-wallet',
        'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
        'npm install',
        'npm run build',
