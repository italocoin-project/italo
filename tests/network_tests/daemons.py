...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'italod@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [italod] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/italo-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=italo.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, italonet=True):
        """Sends fake storage server and italonet pings to the running italod"""
        if storage:
            self.json_rpc("storage_server_ping", { "version_major": 9, "version_minor": 9, "version_patch": 9 })
        if italonet:
            self.json_rpc("italonet_ping", { "version": [9,9,9] })
