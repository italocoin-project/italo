...
namespace
{
    ...
    make_miner_transaction(cryptonote::account_public_address const& to)
    {
        ...
        if (!cryptonote::construct_miner_tx(0, 0, 5000, 500, 500, tx, cryptonote::italo_miner_tx_context::miner_block(cryptonote::FAKECHAIN, to)))
        ...
        bool per_output_unlock)
    {
        ...
        for (auto const& source : sources)
        {
            ...
            if (num_outputs_to_use > 0 && is_miner_tx)
              --num_outputs_to_use; // NOTE:(italo): Don't try to transfer the governance reward which is always the last one
        ...
        cryptonote::italo_construct_tx_params tx_params;
